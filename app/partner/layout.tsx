...
export default function PartnerLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-canvas flex items-center justify-center">
        <div className="animate-spin rounded-full h-8 w-8 border-2 border-line border-t-ink" />
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-canvas">
      <header className="bg-white/80 backdrop-blur-lg border-b border-line/60 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <Link href="/partner" className="flex items-center gap-2.5 text-xl font-bold text-ink">
                <div className="h-8 w-8 rounded-xl bg-ink text-signal-ink flex items-center justify-center flex-shrink-0">
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center space-x-2 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                        isActive
                          ? "bg-canvas-sunk text-signal"
                          : "text-ink-2 hover:text-ink hover:bg-canvas-sunk"
                      }`}
                    >
                      <Icon className={`h-4 w-4 flex-shrink-0 ${isActive ? "text-signal" : ""}`} />
            ...
            <div className="flex items-center space-x-4">
              <span className="hidden sm:block text-sm text-ink-2">{partner.company_name}</span>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => {
                ...
                className="hidden sm:flex items-center gap-1 text-ink-2 hover:text-ink-2"
              >
                    ...
                    <nav className="flex flex-col space-y-1">
                      {navigationItems.map((item) => {
                        ...
                        return (
                          ...
                          >
                            <Icon className={`h-5 w-5 ${isActive ? "text-signal" : ""}`} />
                            <span>{item.label}</span>
                          </Link>
                        )
                      })}
  ...
  return (
    <div className="min-h-screen bg-canvas flex items-center justify-center p-4">
      <div className={`max-w-md w-full ${config.bg} ${config.border} border rounded-2xl p-8 text-center`}>
        ...
        <h1 className="text-xl font-bold text-ink mb-2">{config.title}</h1>
        <p className="text-ink-2 mb-6">{config.description}</p>
        <Button
          variant="outline"
          onClick={() => {
            sessionAuth.clearSession()

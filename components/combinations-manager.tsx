...
import { Loader2, Plus, Trash2, X, GripVertical, ImageIcon } from "lucide-react"
...
export function CombinationsManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Ошибка",
        description: error instanceof Error ? error.message : "Не удалось создать сочетание",
        variant: "destructive",
      })
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Диалог добавления сочетания */}
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Добавить сочетание</DialogTitle>
          </DialogHeader>

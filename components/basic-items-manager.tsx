...
export function BasicItemsManager() {
    ...
    const uploadImage = async (file: File): Promise<{ success: boolean; url?: string; error?: string }> => {
        try {
            ...
            const result = await api.post('/api/upload-image', formData)
    ...
    const handleCopy = async (id: number) => {
        try {
            await api.post("/api/basic-items/copy", {
                id
            })
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    {item.image_url ? (
                                        ...
                                        <div
                                            className="aspect-square w-full bg-gray-100 rounded-md flex items-center justify-center">
                                            <ImageIcon className="h-8 w-8 text-gray-400"/>
                                        </div>
                                    )}
                                    <div>
                                        <span
                                            className="font-medium">Название (EN):</span> {item.name_en || "Не указано"}
                                    </div>
                                    {item.description && (
                                        <div>
                                    ...
                                    {item.materials && item.materials.length > 0 && (
                                        <div>
                                            <span
                                                className="font-medium">Материалы:</span> {item.materials.map((m) => m.name_ru).join(", ")}
                                        </div>
                                    )}
                                </div>
                            </CardContent>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="space-y-4 py-4 pb-20">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {imagePreview && (
                                        <div
                                            className="mt-2 relative w-48 h-48 mx-auto overflow-hidden rounded-md bg-gray-50">
                                            <Image src={imagePreview || "/placeholder.svg"} alt="Preview" fill
                                                   className="object-contain"/>
                                            <Button
                                                type="button"
                                                variant="destructive"

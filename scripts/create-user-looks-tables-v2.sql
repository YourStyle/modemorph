-- Create user_looks table for storing user's saved outfits
CREATE TABLE IF NOT EXISTS user_looks (
  id SERIAL PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  name VARCHAR(255) NOT NULL,
  description TEXT,
  items JSONB NOT NULL DEFAULT '[]'::jsonb, -- Array of objects: [{"type": "user", "id": 1}, {"type": "basic", "id": 10}]
  image_url TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Create looks_sections table for organizing looks into collections
...
-- Create junction table for looks and sections (many-to-many relationship)
...
CREATE POLICY "Users can insert section_looks for their sections" ON section_looks
  FOR INSERT WITH CHECK (
    ...
    EXISTS (
      SELECT 1 FROM user_looks 
      WHERE user_looks.id = section_looks.look_id 
      AND user_looks.user_id = auth.uid()
    )
  );

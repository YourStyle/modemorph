...
TEMP_RANGES: dict[str, tuple[int, int]] = {
    ...
    # Единственный аксессуар с погодным окном: шарф зимний по смыслу. Сумка,
    # часы, украшения, ремень и очки намеренно НЕ здесь — infer_temp_range
    # вернёт (None, None), temp_ok фейлит-открыто, и это правильно: они от
    # погоды не зависят. Шапка тоже fail-open здесь, но её зимнее окно уже
    # ловится по имени в outfit_compat._COLD_ACCESSORY_KEYWORDS.
    "scarf": (-30, 12),

...
# Цены — те же, что обещает лендинг modemorph.site (миграция 046). Раньше здесь
# стояли 699 и 6 990 из «Тарифы_V2», и приложение брало на 100 и 1000 ₽ больше,
# чем звал лендинг.
_PRICE = {"weekly": 299, "monthly": 599, "yearly": 5990}

# Пол маржи худшего случая. Был 40%, опущен до 35% осознанно 17.09.2026, когда
# годовой встал на 5 990 вместо 6 990: при себестоимости включённого 3 893 ₽ это
# даёт 35,0%, и цену с лендинга иначе не провести.
#
# Годовой и по построению самый тонкий: он обязан быть просторнее месячного при
# меньшей цене за месяц, и его маржа ниже — это плата за то, чтобы у апгрейда был
# смысл. Пол ловит не «стало хуже», а «щедрость вышла из берегов».
#
# Ниже опускать нельзя без пересмотра потолков: разовое предложение на годовой
# (4 999 ₽) даёт уже 22,1% при поле скидок в 20%.
_MARGIN_FLOOR = 35.0

...
# Вещи без разметки пола (в каталоге 2718 'unisex' и 1317 NULL) шли в оба ведра,
# и в мужские образы попадали балетки — жалоба с прода 2026-08-18. Разметке
# верим, а неразмеченное отсеиваем по названию: это дёшево и ловит ровно те
# случаи, где ошибка видна на кадре.
_FEMALE_ONLY_WORDS = (
    "женск", "для женщин", "для девушек", "для девочек",
    "юбка", "платье", "блузка", "сарафан", "туника", "боди",
    "балетк", "туфли", "босонож", "лодочк", "шпильк", "мэри джейн",
    "колготк", "бюстгальтер", "легинс",
    "women", "woman", "girl", "ballet flat", "heels",
)


def _is_female_only(it: dict) -> bool:
    """Явно женская вещь по названию — нужна там, где пол в каталоге не проставлен."""
    return any(w in (it.get("name") or "").lower() for w in _FEMALE_ONLY_WORDS)

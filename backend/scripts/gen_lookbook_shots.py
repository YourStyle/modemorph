...
async def main() -> None:
    ...
    # Пустая строка в --ids — это НЕ «идентификаторы не заданы». Раньше
    # `--ids ""` молча превращался в режим «сгенерировать всем, у кого нет
    # кадра»: подстановка списка в вызывающем скрипте провалилась, аргумент
    # приехал пустым, и прогон сделал 27 лишних кадров на $2.06. Аргумент был
    # передан — значит человек хотел ограничить набор, и пустой список означает
    # «нечего делать», а не «делай всё».
    if args.ids is not None and not args.ids.strip():
        sys.exit("--ids передан пустым: нечего генерировать (уточни список)")

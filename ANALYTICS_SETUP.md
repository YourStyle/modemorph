...
✅ Добавлена возможность экспорта данных в Excel на страницах админ-панели:

### На странице Аналитики (/admin/analytics)
- Кнопка "Экспорт в Excel" в правом верхнем углу
- Создаёт файл `analytics_YYYY-MM-DD.xlsx` с 3 листами:
  - **Сводка**: все метрики аналитики
  - **Воронка**: данные воронки конверсии
  - **Динамика**: временной ряд событий за 30 дней

### На странице Пользователей (/admin/users)
- Кнопка "Экспорт в Excel" в правом верхнем углу
- Создаёт файл `users_activity_YYYY-MM-DD.xlsx` с 4 листами:
  - **Пользователи**: список пользователей с активностью
  - **Метрики**: сводные метрики (MAU, DAU)
  - **Активность**: график активности за 30 дней
  - **Регистрации**: график регистраций за 30 дней

Подробнее см. `docs/EXCEL_EXPORT.md`

## Что дальше?

1. Создайте таблицу `user_events` в Supabase (см. выше)
2. Откройте `/admin/analytics` и посмотрите на аналитику
3. Используйте кнопку "Экспорт в Excel" для скачивания данных
4. Добавьте дополнительный трекинг там, где нужно (используя хук `useAnalytics`)

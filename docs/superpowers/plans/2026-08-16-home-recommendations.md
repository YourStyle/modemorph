...
## Замер потерь 2026-08-16 (Task 5) — потерь НЕТ

Прогон `scripts/measure_outfit_dropoff.py` на проде, последние 300 генераций:
...
всего образов (без gap): 5904
ниже порога 3: 0 (0%)

разделов за прогон: {4: 7, 5: 24, 6: 96, 7: 160, 8: 8}
образов за прогон:  пик на 21, основная масса 16-24
```

**Гипотеза о потерях опровергнута.** Отсев `len(items) >= 3` не выбрасывает
ни одного образа. Система выдаёт 6–7 разделов по ~20 образов — ровно то, что
просит промпт (5–7 разделов, 15–25 образов). Лимитов на пути к экрану нет:
API берёт последние 7 дат (`recommendations.py:437`), фронт отсеивает только
пустые разделы (`page.tsx:361`).

Счётчики «1 образов» и «3 образов» на скриншотах пользователя — это счётчики
ВНУТРИ раздела, а не итог по главной. Три образа в разделе «На прогулку» —
норма по промпту.

**Следствие: второй план на «починку потерь и плотность» не нужен.** Часть 4
спеки закрыта отрицательным результатом. Ощущение «мало образов» порождала
gap-секция с надписью «1 образов», и это чинит Task 4, а не увеличение
генерации.
